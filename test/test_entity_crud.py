...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('test_database_changes.log'),
...
class TestEntityCRUD:    
    ...
    def setup_and_teardown(self):
        ...
        logger.info(f"Initial entities: {[entity['name'] for entity in initial_entities]}")
        
        # Store test entities that will be created for cleanup
        self.test_entity_names = []
        
        yield
        
        # Cleanup: Delete any test entities that might still exist
        logger.info(f"=== TEST CLEANUP ===")
        for entity_name in self.test_entity_names:
        ...
        final_entities = get_all_entities()
        logger.info(f"Final entity count: {final_count}")
        logger.info(f"Final entities: {[entity['name'] for entity in final_entities]}")
    ...
    def track_entity(self, name: str):
        """Helper method to track entities for cleanup"""
        self.test_entity_names.append(name.strip().title())
    ...
    def test_get_entity_by_name(self):
        ...
        non_existent = get_entity_by_name("Non Existent Club")
        logger.info(f"Non-existent entity result: {non_existent}")
        assert non_existent == {}
    ...
    def test_update_entity_name(self):
        ...
        self.track_entity(original_name)
        self.track_entity(new_name)  # Track both names for cleanup
        ...
        assert old_entity == {}  # Should not find old name
    ...
    def test_delete_entity(self):
        ...
        # Don't track this entity since we're testing deletion
